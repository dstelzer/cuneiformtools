...
class Layout:
	...
	def row_width(self, row):
		...
		space_total = sum(self.spacing for space in row if isinstance(space, Spacer)) # Width of each space
	...
	def render_row(self, row, y):
		if not self.ready: raise ValueError('Not ready!') # Don't try to render anything if we don't have a renderer set up
		...
		if self.justify == Justification.LEFT:
